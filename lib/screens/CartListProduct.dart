...
class _CartListProductState extends State<CartListProduct> {
    ...
    return Scaffold(
      body: Padding(
        ...
        child: cartBox.isEmpty
            ? Center(
                child: Text('No cart here. Please add your product.'),
              )
            : 
  ...
  itemBuilder: (context, index) {
    ...
    return Dismissible(
      ...
      onDismissed: (direction) async {
        final deletedProduct = cartBox.getAt(index);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            action: SnackBarAction(
              label: 'Undo',
              onPressed: () async {
                // Jika pengguna mengklik "Undo", tambahkan kembali produk ke keranjang
                await cartBox.add(deletedProduct!);
              },
            ),
          ),
        );
  ...
  String formatPrice(double price) {
    switch (selectedCurrency) {
      ...
      case Currency.IDR:
        return 'Rp${(price * 15433.60).toStringAsFixed(2)}';
    }
  }
  ...
  String _getFormattedTime(DateTime time, TimeZone timeZone) {
    switch (timeZone) {
      ...
      case TimeZone.SouthKr:
        return '${time.hour + 2} :${time.minute}:${time.second} South Korea';
